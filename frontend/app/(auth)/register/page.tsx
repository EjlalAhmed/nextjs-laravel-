    ...
    <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,_#f9f5ea,_#efe4cc)] px-4">
      <div className="w-full max-w-md rounded-[24px] border border-stone-300 bg-[#fffdfa] p-8 shadow-[0_16px_50px_-20px_rgba(74,47,22,0.35)]">
        <div className="mb-6 text-center">
          <p className="text-sm uppercase tracking-[0.35em] text-stone-500">New Account</p>
          <h1 className="mt-2 text-3xl font-semibold text-stone-800">Create account</h1>
          <p className="mt-2 text-sm text-stone-600">Begin your elegant expense tracking journey.</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <input className="w-full rounded-xl border border-stone-300 bg-white p-3 text-stone-700 outline-none ring-0" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} />
          ...
          <input className="w-full rounded-xl border border-stone-300 bg-white p-3 text-stone-700 outline-none ring-0" type="password" placeholder="Confirm Password" value={passwordConfirmation} onChange={(e) => setPasswordConfirmation(e.target.value)} />
        ...
        <p className="mt-5 text-center text-sm text-stone-600">
          Already have an account? <a className="font-medium text-stone-800 underline" href="/login">Sign in</a>
        </p>
      </div>
    </div>
  );
